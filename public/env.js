...
//updates profile name, age, picture and bio using current userProfile data 
const userProfileUpdate = () => {
    ...
    //sets center icon image to user pic
...
function validateProfileForm() {
    ...
    //checks form validation for input fields, and returns alerts where not valid
    if (!firstName.checkValidity()) {
        alert('Please enter your name');
        //if form DOB input is valid, check user is old enough (>17)
...
//set user profile data, set up suitor ("matching") display and make it visible
...
//on confirming form input valid, proceed with profile creation and presenting potential matches
const getMatchingFunction = () => {
    ...
    //required to stop "submit" button reloading the page whilst keeping in-form validation cues
    return false;
}

//(used on profile rejection) shows next profile or goes to "matchingComplete" function if none remain
...
//(used on profile like) displays match pane, adds to match list & triggers email before proceeding to next profile (or "matching complete" if no more)
...
//updates modal match display using the index profile details
...
//displays the selected (index) profile in the matching pane
...
//store X coordinate of mouse button down event
...
$(document).ready(function () {
    ...
    let now = new Date();
    $('.datepicker').datepicker({ yearRange: [1940,2020], defaultDate: now });
    console.log('Ready');
